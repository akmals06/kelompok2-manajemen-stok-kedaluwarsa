...
export default function LoginClient() {
  ...
  const handleForgotSend = async () => {
    ...
    try {
      ...
      const kodeOtp = res.data?.otp;
      if (kodeOtp) {
        setOtp(kodeOtp.split(''));
      } else {
        setOtp(['', '', '', '', '', '']);
      }
      setActiveView('forgot-2');
      setAlertOtp({ type: 'success', msg: 'Kode verifikasi berhasil dibuat. Silakan klik Verifikasi Kode.' });
    } catch (err) {
      const msg = err.response?.data?.message || 'Gagal membuat kode verifikasi.';
      ...
      <div className="login-card">
        ...
        <div className={`login-view ${activeView === 'forgot-1' ? 'active' : ''}`}>
          ...
          <p className="view-desc">
            Masukkan alamat email akun Anda untuk mendapatkan kode verifikasi 6 digit.
          </p>
        ...
        <div className={`login-view ${activeView === 'forgot-2' ? 'active' : ''}`}>
          ...
          <p className="view-title">Verifikasi Kode OTP</p>
          <p className="view-desc">
            Kode 6 digit sudah terisi otomatis. Klik tombol di bawah untuk melanjutkan.
