const stokRepo = require('./stock.repository');
...
const buatStokKeluar = async (idPengguna, data) => {
  ...
  if (idBatch) {
    ...
    if (['KEDALUWARSA', 'DIARSIPKAN', 'DITOLAK'].includes(batch.status_batch)) {
      throw Object.assign(new Error('Batch sudah kedaluwarsa, diarsipkan, atau ditolak'), { statusCode: 422 });
    }
    if (new Date(batch.tanggal_kedaluwarsa) <= new Date()) {
      throw Object.assign(new Error('Batch telah melewati tanggal kedaluwarsa'), { statusCode: 422 });
    }
    if (batch.jumlah_sisa < jumlah) {
